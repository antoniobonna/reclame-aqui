...
DELETE FROM reclame_aqui.reclamacoes_nao_avaliadas_stg WHERE id in (SELECT reclamacao_id FROM reclame_aqui_dw.reclamacao);
...
SELECT distinct datetime, date(datetime), date_part('week', datetime),date_part('month', datetime),date_part('year', datetime)
	FROM reclame_aqui.reclamacoes_nao_avaliadas_stg WHERE datetime not in (SELECT datetime FROM reclame_aqui_dw.datetime)
ORDER BY 1;
...
FROM reclame_aqui.reclamacoes_nao_avaliadas_stg WHERE not cidade is null
EXCEPT
...
FROM reclame_aqui.reclamacoes_nao_avaliadas_stg
ORDER BY 1;
...
-- reclame_aqui_dw_dw.reclamacoes_nao_avaliadas

\! echo "Carregando dados na tabela fato reclamacoes_nao_avaliadas..."

COPY(
SELECT distinct e.empresa_id, d.datetime, c.cidade_id, r.reclamacao_id, f.url
	FROM reclame_aqui.reclamacoes_nao_avaliadas_stg f
...
) to '/home/ubuntu/dump/reclamacoes_nao_avaliadas.txt';
COPY reclame_aqui_dw.reclamacoes_nao_avaliadas FROM '/home/ubuntu/dump/reclamacoes_nao_avaliadas.txt';

VACUUM ANALYZE reclame_aqui_dw.reclamacoes_nao_avaliadas;

\! echo "Truncate Staging"

TRUNCATE reclame_aqui.reclamacoes_nao_avaliadas_stg;
